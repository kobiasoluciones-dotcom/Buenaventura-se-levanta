...
function detectarPlataforma(url?: string | null) {
  if (!url) return null;
  let host = "";
  try { host = new URL(url).hostname.replace(/^www\.|^web\.|^m\./, ""); } catch { return null; }
  if (host.includes("tiktok.com")) return { label: "TikTok", clase: "plataforma-tiktok" };
  if (host.includes("instagram.com")) return { label: "Instagram", clase: "plataforma-instagram" };
  if (host.includes("facebook.com") || host.includes("fb.watch")) return { label: "Facebook", clase: "plataforma-facebook" };
  if (host.includes("youtube.com") || host.includes("youtu.be")) return { label: "YouTube", clase: "plataforma-youtube" };
  if (host.includes("whatsapp.com") || host.includes("wa.me")) return { label: "WhatsApp", clase: "plataforma-whatsapp" };
  if (host.includes("x.com") || host.includes("twitter.com")) return { label: "X", clase: "plataforma-x" };
  return { label: host || "Enlace externo", clase: "plataforma-generica" };
}

function MarcaPlataforma({ url, className = "" }: { url?: string | null; className?: string }) {
    ...
    <main>
      ...
      <section className="today-section" id="hoy">
        ...
        {noticias.length > 0 ? <div className="shell editorial-grid">
          ...
          {noticias[1] && <article className="side-story side-story-top">{noticias[1].archivo ? <img src={noticias[1].archivo} alt={noticias[1].titulo} /> : <MarcaPlataforma url={noticias[1].url_externa} className="side-story-empty" />}<div><p className="story-meta">{noticias[1].nivel_confianza} · {formatDate(noticias[1].fecha_publicado)}</p><h3>{noticias[1].titulo}</h3><p>{noticias[1].descripcion}</p>{noticias[1].url_externa && <a className="text-link" href={noticias[1].url_externa} target="_blank" rel="noreferrer">Ver publicación ↗</a>}</div></article>}
